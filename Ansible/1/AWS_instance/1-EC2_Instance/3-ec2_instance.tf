...
resource "aws_instance" "ec2_instance" {
  ...
  vpc_security_group_ids = [aws_security_group.Ansible-c-sg.id]
...
resource "null_resource" "name" {
  ...
  # Connect to the instance using the .pem and the user
  connection {
    type = "ssh"
    user = "admin"
    host = aws_instance.ec2_instance.public_ip
  ...
  # Set permissions and run the installation-script.sh file
  provisioner "remote-exec" {
    inline = [
      "sudo chmod +x /tmp/bash_script/install_ansible.sh",
      "sh /tmp/bash_script/install_ansible.sh",
    ]
  }

  # Copy local key to remote host
  provisioner "file" {
    source      = "~/.ssh/ansible.pub"
    destination = "/home/admin/.ssh/ansible.pub"
  }
